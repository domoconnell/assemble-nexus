	...
	return (
		<>
			<Hero
				...
				title={hero.title ? <RichText html={hero.title} /> : "The next few nights."}
				subtitle={hero.subtitle ? <RichText html={hero.subtitle} /> : "Concerts, panels, launches, and the odd weird night out. A mix of our own shows and what's hired the rooms."}
				...
				{events.length === 0 ? (
					<div className="rounded-xl border border-foreground/10 bg-card p-10 text-center max-w-md mx-auto">
						...
						<div className="mt-3 text-sm text-muted-foreground">
							{empty.body ? (
								<RichText html={empty.body} />
							) : (
								<p>Check back soon. We&rsquo;re putting the next few months together.</p>
							)}
						</div>
					</div>
				) : (
